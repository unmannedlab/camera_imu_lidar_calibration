...
void UpdaterLidarOdometry::updateScan2Scan(State *current_state, relativePose lodom, bool &did_update) {
    ...
    if(_options.do_chi2_check) {
        if (chi2 > chi2_check/10) {
            printf(BOLDRED "[Update : LiDAR] unsuccessful updateScan2Scan \n" RESET);
            did_update = false;
            return;
        }
    }
    ...
    printf(BOLDGREEN "[Update : LiDAR] successful updateScan2Scan \n" RESET);
    did_update = true;
}

void UpdaterLidarOdometry::updateScan2GlobalMapRotation(State *current_state, Eigen::Matrix4d L1_T_Lk, Eigen::Matrix4d G_T_I1, double timestamp, bool& did_update) {
    /// G_T_I1
    ...
    /// Add the clone k: IMU pose corresponding curr update time
    ...
    /// Assign Jacobian blocks
    /// H1 block, corresponding to res1
    Eigen::MatrixXd H1_xk_rot = Eigen::MatrixXd::Zero(3, 3);
    Eigen::MatrixXd H1_xk_trans = Eigen::MatrixXd::Zero(3, 3);
    H1_xk_rot = I_R_L.transpose();
    H1_xk_trans = -I_R_L.transpose()*I1_R_G*Ik_R_G.transpose();

    Eigen::MatrixXd H_x = Eigen::MatrixXd::Zero(3, total_hx);
    H_x.block(0, 0, 3, 3) = H1_xk_rot;
    H_x.block(0, 3, 3, 3) = H1_xk_trans;

    Eigen::MatrixXd H1_xc_rot = Eigen::MatrixXd::Zero(3, 3);
    Eigen::MatrixXd H1_xc_trans = Eigen::MatrixXd::Zero(3, 3);
    if(current_state->_options.do_calib_lidar_imu_extrinsic) {
        H1_xc_rot = I_R_L.transpose()*(I1_R_G*Ik_R_G.transpose()-Eigen::MatrixXd::Identity(3, 3));
        H_x.block(0, 6, 3, 3) = H1_xc_rot;
        H_x.block(0, 9, 3, 3) = H1_xc_trans;
    }

    /// Assign measurement residual
    Eigen::Matrix<double, 3, 1> res_rot =  Log_so3(L1_R_Lk * L1_R_Lk_hat.transpose());
    Eigen::MatrixXd R = std::pow(_options.noise_rotation, 2)*Eigen::MatrixXd::Identity(3, 3);
    ...
    if(_options.do_chi2_check) {
        if (chi2 > chi2_check/10) {
            printf(BOLDRED "[Update : LiDAR] unsuccessful updateScan2GlobalMapRotation \n" RESET);
            did_update = false;
            return;
        }
    }
    ...
    printf(BOLDGREEN "[Update : LiDAR] successful updateScan2GlobalMapRotation \n" RESET);
    did_update = true;
}

void UpdaterLidarOdometry::updateScan2GlobalMapTranslation(State *current_state, Eigen::Matrix4d L1_T_Lk, Eigen::Matrix4d G_T_I1, double timestamp, bool& did_update) {
    /// Initial IMU pose
    Eigen::Matrix<double, 3, 3> G_R_I1 = G_T_I1.block(0, 0, 3, 3);
    ...
    if(_options.do_chi2_check) {
        if (chi2 > chi2_check/10) {
            printf(BOLDRED "[Update : LiDAR] unsuccessful updateScan2GlobalMapTranslation\n" RESET);
            did_update = false;
            return;
        }
    }
    ...
    printf(BOLDGREEN "[Update : LiDAR] successful updateScan2GlobalMapTranslation\n" RESET);
    did_update = true;
}